...
export class VersionController extends React.Component {
  ...
  constructor(props) {
    ...
    database()
      .ref('accounts/' + global.user.name + '/version')
      .set(this.versionCode, function (error) {
      ...
      .on('value', snapshot => {
        ...
        if (major) {
          this.setState({
            canBeIgnored: false,
            showDialog: true,
            updateInfo: data.majorText,
          });
        } else if (minor && !major) {
          this.setState({
            canBeIgnored: false,
            showDialog: true,
            updateInfo: data.minorText,
          });
        } else if (patch && !minor && !major) {
          this.setState({
            canBeIgnored: true,
            showDialog: true,
            updateInfo: data.patchText,
          });
        }
      });
  }
      ...
      <Dialog
        ...
        footer={
          <DialogFooter style={{flexDirection: 'row'}}>
            <DialogButton text="Не обновлять" onPress={() => this.dismiss()} />

            <DialogButton text="Обновить" onPress={() => this.openLink()} />
          </DialogFooter>
        }>
        <DialogTitle title="Доступна новая версия" />
        <DialogContent>
