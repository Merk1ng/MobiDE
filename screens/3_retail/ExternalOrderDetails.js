...
import PopupDialog from 'react-native-popup-dialog/src';
import {
  DialogButton,
  DialogFooter,
  DialogTitle,
  ...
  renderGoodItem = itemRow => {
    return (
      <TouchableOpacity
        ...
        <Text numberOfLines={2} style={styles.item33}>
          {itemRow.item.name}
        </Text>
        <Text style={styles.itemC}>
          {itemRow.item.quantity +
            ' ' +
            itemRow.item.unit_name +
            '. \n' +
            Number.parseFloat(itemRow.item.price).toFixed(2) +
            ' ₽'}
          &gt;
        </Text>
      </TouchableOpacity>
    ...
    return (
      <AppHeader
        ...
        rightIconAction={
          this.IS_NEW
            ? this._save
            : () => this.setState({dialog_createPurchase: true})
        }
      />
    );
  }
    ...
    if (this.state.allowedAccept <= false) {
      ...
      return (
        <PopupDialog
          ...
          <DialogTitle title={'Создать ПТУ на основании?'} />
        </PopupDialog>
      );
    }
  }
