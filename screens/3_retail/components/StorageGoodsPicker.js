...
import {Icon, SearchBar, Divider, Button, ListItem} from '@rneui/themed';
import Dialog, {
  DialogContent,
  DialogFooter,
  DialogButton,
...
export class StorageGoodsPicker extends React.Component {
  ...
  clickItem(good) {
    ...
    if (good.items) {
      this.setState({
        breadcrumbs: this.state.breadcrumbs.concat([
          JSON.parse(JSON.stringify(this.state.displayGoods)),
        ]),
    ...
    if (good.name === backLabel) {
      ...
      --nbc.length;
      this.setState({
        displayGoods: this.state.breadcrumbs[this.state.breadcrumbs.length - 1],
        breadcrumbs: nbc,
      });
      return;
    }
  ...
  askQuantity(val) {
    if (this.state.item && !this.state.item.measurement) {
      this.setState({
        item: CommonFunctions.getReference(
          this.state.item,
  ...
  render() {
    ...
    console.log(JSON.parse(JSON.stringify(global.hgoods) || '{}'));
            ...
            <FlatList
              ...
              renderItem={(rowData, index) => (
                <ListItem
                  ...
                  <ListItem.Content>
                    <ListItem.Title>{rowData.item.name}</ListItem.Title>
                  </ListItem.Content>
                </ListItem>
              )}
