...
export class FoodServiceOrderDetails extends React.Component {
  ...
  willFocus() {
    ...
    if (!doc) {
      ...
      doc.organization = CommonFunctions.getReference(
        doc.organization,
  ...
  _saveDocument() {
    ...
    let promise = new Promise((resolve, reject) => {
      database()
        .ref('i/' + global.user.name + '/InternalOrders/' + this.state.id)
        .set(this.state, function (success) {
          resolve('success');
        });
    });
