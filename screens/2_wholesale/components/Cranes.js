...
import {ListItem, SearchBar, Divider, Button} from '@rneui/themed';
import Dialog, {
  DialogContent,
  DialogFooter,
  DialogButton,
...
export class Cranes extends React.Component {
      ...
      <Dialog
        ...
        footer={
          <DialogFooter style={{flexDirection: 'row'}}>
            ...
            />
            <DialogButton text="Готово" onPress={() => this.confirm()} />
          </DialogFooter>
        }>
        <DialogTitle title="Создание нового вида товара" />
        <DialogContent>
  ...
  SearchFilterFunction = text => {
    ...
    if (
      !Array.isArray(this.state.displayGoods) ||
      this.state.displayGoods.length === 0
    ) {
  ...
  render() {
    ...
    return (
      <View style={[{backgroundColor: '#fff'}]}>
        ...
        <FlatList data={this.props.value} renderItem={this.renderItem} />
