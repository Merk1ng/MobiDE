...
export class SurveyDetails extends DocumentDetails {
  ...
  _save = () => {
    ...
    if (!this.state.items) {
      CommonFunctions.showAlert(
        'Ошибка заполнения документа',
        'Не выбрано ни одного сорта пива',
      );
      return;
    }
    ...
    AsyncStorage.getItem(this.ASYNC_STORAGE_KEY).then(result => {
      ...
      AsyncStorage.setItem(this.ASYNC_STORAGE_KEY, JSON.stringify(list)).then(
        res => {
