    ...
    if (!Array.isArray(global.documents) || global.documents.length === 0) {
      Promise.all([
        ...
        .then(results => {
          ...
          if (
            !Array.isArray(global.documents) ||
            global.documents.length === 0
          ) {
        ...
        uploadTask.on(
          ...
          function (snapshot) {
            ...
            if (progress >= Uploading.progress_state) {
              ...
              ctx.setState({
                progress: Number.parseFloat(Uploading.progress_state),
              });
            }
          },
          function (error) {
